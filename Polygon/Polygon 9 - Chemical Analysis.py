...
def outside_rec(formula, multiplier, elements = None):
...
def brackets_rec(formula, multiplier, elements = None):

	if elements is None:
		elements = {}

	outside_rec(formula, 1, elements)

	brackets = re.findall(r'[A-Z]*[a-z]*[\(\[](\S+)[\)\]](\d*)', formula)

	print(brackets, "set of brackets")

	if brackets:
		print(brackets[0][0])
		outside_rec(brackets[0][0], multiplier, elements)
		for item in brackets:
			j = 1 if item[1] == '' else int(item[1])
			for y in brackets_rec(item[0], j, elements):
				print(y, "y", multiplier)
...
def atoms(formula, limit):
	...
	elements = brackets_rec(formula, 1)
