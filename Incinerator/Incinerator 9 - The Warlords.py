...
class Warrior(object):
	...
	def make_attack(self, defender, next_unit_attacker = None, next_unit_defender = None):
		...
		if log_print:
			print(f'{self.__class__.__name__} does {attack} damage to {defender.__class__.__name__}, {defender.__class__.__name__} health is now {defender.health}')
		if hasattr(self, 'vampirism'):
			self.bloodsuck(defender)
		if hasattr(self, 'piercing'):
			self.piercing_attack(next_unit_defender)
		if hasattr(next_unit_attacker, 'heal_power'):
			next_unit_attacker.heal(self)
		if defender.health <= 0:
			defender.is_alive = False
			if log_print:
				print(f'{defender.__class__.__name__} loses the fight and dies!')
...
class Vampire(Warrior):
	...
	def bloodsuck(self, defender):
		attack = self.apply_defence(defender)
		vampire_damage = int((self.vampirism * attack / 100)//1)
		if not self.health + vampire_damage >= self.full_health:
			self.take_damage(-vampire_damage)
			if log_print:
				print(f'{self.__class__.__name__} sucked {vampire_damage} from {defender.__class__.__name__}. Vampire health is now {self.health}')
...
class Army():
	...
	def add_units(self, unit_type, no_of_units):
		...
		if any([x for x in self.units if x.__class__.__name__ == 'Warlord']):
	...
	def move_units(self):
		...
		if not self.has_warlord:
			return
		warlord = [x for x in self.units if isinstance(x, Warlord)]
		lancers = [x for x in self.units if isinstance(x, Lancer)]
		other_warriors = [x for x in self.units if x.attack > 0 and not isinstance(x, Lancer) and not isinstance(x, Warlord)]
		# other_warriors.sort(key = lambda x: x.attack, reverse = True)
		healers = [x for x in self.units if isinstance(x, Healer)]

		fighters = lancers + other_warriors
		fighters_ordered = fighters[:1] + healers + fighters[1:]

		self.units = fighters_ordered + warlord
	...
	def fight(self, army_1, army_2):
		while army_1.is_army_alive() and army_2.is_army_alive():
			if fight(army_1.units[0], army_2.units[0], army_1.next_unit(), army_2.next_unit()):
				if log_print:
					print(f' Army 2 Loses and loses {army_2.units[0].__class__.__name__}')
				army_2.remove_dead()
				army_2.move_units()
			else:
				if log_print:
					print(f' Army 1 Loses and loses {army_1.units[0].__class__.__name__}')
				army_1.remove_dead()
				army_1.move_units()
		else:
...
def fight(unit_1 = None, unit_2 = None, unit_3 = None, unit_4 = None):
	...
	while True:
		if log_print:
			print(f'Attacker = {unit_1.__class__.__name__}, Defender = {unit_2.__class__.__name__}, Attacker_Next = {unit_3.__class__.__name__}, Defender_Next = {unit_4.__class__.__name__}')
		unit_1.make_attack(unit_2, unit_3, unit_4)
		if not unit_2.is_alive:
			return True
		if log_print:
			print(f'Attacker = {unit_2.__class__.__name__}, Defender = {unit_1.__class__.__name__}, Attacker_Next = {unit_4.__class__.__name__}, Defender_Next = {unit_3.__class__.__name__}')
